...
def get_datetime(exif) -> datetime:
    ...
    try:
        dt = parser.parse(
            extract_attribute(
                exif, ExifId.DATETIME.value, default_value=str(datetime.now())
            )
        )
    except ValueError:
        logger.info(
...
def get_focal_length(exif: dict[str, str]) -> tuple[str, str]:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"ValueError: 不存在焦距：{focal_lengths} : {e}")
        try:
            focal_length_in_35mm_film: str = (
                focal_lengths[1] if focal_length else DEFAULT_VALUE
            )
        except IndexError as e:
            logger.error(f"ValueError: 不存在 35mm 焦距：{focal_lengths} : {e}")
...
class ImageContainer:
    def __init__(self, path: Path):
        ...
        # 修正图像方向
        self.orientation = (
            self.exif[ExifId.ORIENTATION.value]
            if ExifId.ORIENTATION.value in self.exif
            else 1
        )
        ...
        if "GPSPosition" in self.exif:
            ...
            self._param_dict[GEO_INFO_VALUE] = str.join(
                " ",
                extract_gps_lat_and_long(
                    (self.exif.get("GPSLatitude"), self.exif.get("GPSLongitude"))
                ),
            )
        else:
    ...
    def get_attribute_str(self, element: Element) -> str:
        """
        通过 element 获取属性值
        :param element: element 对象有 name 和 value 两个字段，通过 name 和 value 获取属性值
        :return: 属性值字符串
        """
        if element.name in self._param_dict:
            return self._param_dict[element.name]

        if element is None or element.name == "":
            return ""
        if element.name == CUSTOM_VALUE:
            self.custom = element.value
            return self.custom
        elif element.name in self._param_dict:
            return self._param_dict[element.name]
        else:
            return ""
    ...
    def get_param_str(self) -> str:
        ...
        focal_length = (
            ...
            if self.use_equivalent_focal_length
